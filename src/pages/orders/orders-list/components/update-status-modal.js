...
const UpdateStatusModal = ({ rowSelection, refreshList }) => {
	...
	return (
		<>
			<Col span={8}>
				<Button onClick={() => setStatusVisible(true)} type="primary" disabled={rowSelection?.selectedRowKeys?.length === 0}>
					{rowSelection?.selectedRowKeys?.length === 0 ? `Select rows to update status` : `Update status for ${rowSelection?.selectedRowKeys?.length} order${rowSelection?.selectedRowKeys?.length > 1 ? 's' : ''}`}
				</Button>
			</Col>
			<Modal
