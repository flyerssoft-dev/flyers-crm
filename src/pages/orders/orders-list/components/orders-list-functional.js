...
const OrdersListFunctional = React.memo(() => {
	...
	const getOrders = useCallback(() => {
		dispatch(getApi(ACTIONS.GET_ORDERS, `${SERVER_IP}order?orgId=${globalRedux?.selectedOrganization?.id}`));
				...
				return (
					<Row gutter={10} justify="center">
						...
						{/* <Popconfirm
							...
							placement="rightTop"
							onConfirm={() => {
								let url = `${SERVER_IP}order/${row._id}?orgId=${globalRedux?.selectedOrganization?.id}`;
