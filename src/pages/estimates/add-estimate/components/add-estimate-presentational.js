...
const AddEstimatePresentational = ({
	state,
	setState,
	handleSubmit,
	columns,
	...
	return (
		<Drawer
			maskClosable={false}
			title={`${editData ? 'Edit' : 'Create'} Estimate`}
			placement="right"
			width={'60%'}
			open={state?.visible}
			destroyOnHidden
			className="add_estimate"
			onClose={showConfirm}
			footer={
				<Row>
					<Col
						...
						{...{
							...
							title: () => (
								<Row justify="space-between">
									<Col xl={24} md={24}>
										<Form labelAlign="left" name="add-estimate" style={{}} form={form} onFinish={handleSubmit} {...layer1FormCol}>
											<Row gutter={[20, 20]}>
												<Col xl={12} md={12}>
													<Form.Item
														label="Customer"
...
export default AddEstimatePresentational;
