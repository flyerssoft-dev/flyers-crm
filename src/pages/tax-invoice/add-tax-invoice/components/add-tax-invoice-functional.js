...
const calculateTaxRateTotalAmount = (record, taxPreference) => {
	console.log("🚀 ~ calculateTaxRateTotalAmount ~ taxPreference:", taxPreference)
...
const AddTaxInvoiceFunctional = () => {
	...
	const selectedCustomer = useMemo(() => {
		if (customerIdValue) {
			return customers?.find((customer) => customer?._id === customerIdValue);
		}
		return null;
	}, [customerIdValue, customers]);

	const isSameState = useMemo(() => {
		if (globalRedux?.selectedOrganization?.stateCode && selectedCustomer?.stateCode) {
			return globalRedux?.selectedOrganization?.stateCode === selectedCustomer?.stateCode;
		}
		return false;
	}, [globalRedux?.selectedOrganization?.stateCode, selectedCustomer?.stateCode]);
	console.log("🚀 ~ globalRedux?.selectedOrganization:", globalRedux?.selectedOrganization?.stateCode, isSameState, selectedCustomer?.stateCode);
	...
	useEffect(() => {
		if (invoiceDetails?.data) {
			form.setFieldsValue({
				customerId: invoiceDetails?.data?.customerId?._id || invoiceDetails?.data?.customerId,
				invoiceDate: moment(invoiceDetails?.data?.invoiceDate),
				dueDate: moment(invoiceDetails?.data?.dueDate),
				poNumber: invoiceDetails?.data?.poNumber,
				poDate: invoiceDetails?.data?.poDate ? moment(invoiceDetails?.data?.poDate) : null,
				vehicleNumber: invoiceDetails?.data?.vehicleNumber || '',
				salesPersonId: invoiceDetails?.data?.salesPersonId?._id || invoiceDetails?.data?.salesPersonId,
				projectId: invoiceDetails?.data?.projectId?._id || invoiceDetails?.data?.projectId,
				placeOfSupply: invoiceDetails?.data?.placeOfSupply || '',
				taxPreference: invoiceDetails?.data?.taxPreference || 'Exclusive',
			});
			setInvoiceInputDetails({
				shippingCharges: invoiceDetails?.data?.shippingCharges || 0,
				roundOff: invoiceDetails?.data?.roundOff || 0,
			});
			setTableData([]);
			generateTableData(invoiceDetails?.data, invoiceDetails?.data?.taxPreference);
		}
	...
	const { billingDetails, shippingDetails } = useMemo(() => {
		...
		if (customerIdValue) {
			...
			shippingDetails = customers?.find((customer) => customer?._id === customerIdValue)?.shippingDetails?.[0] || {};
		}
		return {
			billingDetails,
			shippingDetails,
				...
				if (data.id === rowId) {
					newObj = {
						...
						...(item && {
							...
							totalAmount: sellingPrice * qty + taxAmount,
							taxAmount: taxAmount,
						}),
						[label]: value,
					};
					...
					return {
						...
						...calculateTaxRateTotalAmount(newObj, taxPreferenceValue),
					};
				} else {
					return data;
				}
	...
	const generateTableData = (editData, taxPreferenceValue) => {
		if (editData?.items?.length > 0) {
			const data = editData.items.map((item) => {
				const taxRate = typeof item?.taxId === 'object' ? item?.taxId.taxValue : item?.taxRate;
				const taxId = typeof item?.taxId === 'object' ? item?.taxId.id || item?.taxId._id : item?.taxId;

				return {
					itemId: typeof item.itemId === 'object' ? item.itemId.id || item.itemId._id : item.itemId,
					itemName: item?.itemId?.itemName || '',
					description: item?.description || '',
					hsnSac: item?.hsnsac || item?.itemId?.hsnSac || '',
					qty: item?.qty || 1,
					rate: item?.rate || 0,
					discount: item?.discount || 0,
					discountType: item?.discount > 0 ? 'PERCENT' : 'AMOUNT',
					taxRate: taxRate || 0,
					taxId: taxId || null,
					...
					id: uuidv4(),
					...calculateTaxRateTotalAmount(item, taxPreferenceValue || 'Exclusive'),
				};
			});
			setTableData(data);
		}
	...
	const handleSubmit = (values) => {
		let data = {
			...
			taxType: isSameState ? 'IntraState' : 'InterState',
			...invoiceInputDetails,
		...
		if (invoiceId) {
			let url = `${SERVER_IP}invoice/${invoiceId}?orgId=${globalRedux?.selectedOrganization?._id}`;
			dispatch(putApi(data, 'EDIT_TAX_INVOICE', url));
		} else {
			dispatch(postApi(data, 'ADD_TAX_INVOICE'));
		}
	};

	useEffect(() => {
		if (globalRedux.apiStatus.ADD_TAX_INVOICE === API_STATUS.SUCCESS || globalRedux.apiStatus.EDIT_TAX_INVOICE === API_STATUS.SUCCESS) {
			dispatch(resetApiStatus(invoiceId ? 'EDIT_TAX_INVOICE' : 'ADD_TAX_INVOICE'));
			navigate(-1);
		}
	}, [globalRedux.apiStatus, dispatch, navigate]);

	const loading = useMemo(() => globalRedux.apiStatus.ADD_TAX_INVOICE === API_STATUS.PENDING || globalRedux.apiStatus.EDIT_TAX_INVOICE === API_STATUS.PENDING, [globalRedux.apiStatus]);
