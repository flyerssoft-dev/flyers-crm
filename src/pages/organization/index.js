...
const OrganizationList = () => {
    ...
    return (
        <div className="organization-container">
            ...
            <div className="org-list">
                ...
                <div className="org-grid">
                    {organizationDetails.map((details, index) => {
                        ...
                        const key = details._id || details.id || details.orgNumber || index;
                            ...
                            >
                                <div className="org-card-inner">
                                    ...
                                    <div className="org-info">
                                        <h5 className="org-name">{details?.orgName}</h5>
                                        <p className="org-meta">Organization ID: {details?.id || details?.orgNumber}</p>
                                        <p className="org-meta">GST: {details?.edition || details?.gstin}</p>
                                        {details.createdAt && <p className="org-meta">Created On: {moment(details.createdAt).format('DD/MM/YYYY')}</p>}
                                        {details.userRole && (
                                            <p className="org-meta">
                                                You are an <strong>{details.userRole}</strong> in this organization
                                            </p>
                                        )}
                                    </div>
                                </div>
                        ...
                        return isDefault ? (
                            <Badge.Ribbon key={`default-${key}`} text="Default" placement="start">
                                {cardContent}
                            </Badge.Ribbon>
                        ) : (
                            cardContent
            ...
            <AddModal visible={visible} toggleVisible={toggleVisible} modalTitle="Create Organization" width="40%">
                <AddOrganization refreshList={refreshOrganizationList} closeModal={() => toggleVisible(false)} />
            </AddModal>
        </div>
    );
};
