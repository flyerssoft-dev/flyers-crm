		...
		<div class="invoice-pdf" ref={ref}>
			<div className="invoice-pdf-container">
				<table cellPadding="0" cellSpacing="0">
					<tr class="top">
						<td colSpan="10" style={{ padding: 0, paddingTop: 0 }}>
					...
					<tr class="heading custom-heading">
						...
						<td>SAC NO</td>
						<td>Date</td>
						<td>{rateType === 'Warehouse' ? 'WH' : 'Freight'} Rs</td>
						<td>{rateType === 'Warehouse' ? 'WH' : 'Freight'} KG</td>
						<td>AMC</td>
						<td>Others</td>
						<td>LR Charges</td>
						<td>Total Rs.</td>
					...
					{transportItems?.slice(0, 15)?.map((data, index) => (
						<tr class="item custom-item">
							...
							<td style={{ fontWeight: 'bold' }}>{data?.hsnSac}</td>
							<td style={{ fontWeight: 'bold' }}>{moment(data?.date).format(`${DATE_FORMAT.DD_MM_YYYY}`) || ''}</td>
							<td style={{ fontWeight: 'bold' }}>{parseFloat(data?.frightRs || 0).toFixed(2)}</td>
							<td style={{ fontWeight: 'bold' }}>{data?.frightKG || 0}</td>
							<td style={{ fontWeight: 'bold' }}>{parseFloat(data?.amcAmount || 0).toFixed(2)}</td>
							<td style={{ fontWeight: 'bold' }}>{parseFloat(data?.others || 0).toFixed(2)}</td>
							<td style={{ fontWeight: 'bold' }}>{parseFloat(data?.lrCharges || 0).toFixed(2)}</td>
							<td style={{ fontWeight: 'bold' }}>{parseFloat(data?.totalAmount || 0).toFixed(2)}</td>
						</tr>
					))}
					{/* <tr class="total">
					<td></td>
