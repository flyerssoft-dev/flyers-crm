...
import { API_STATUS, CATEGORIES, CUSTOMER_TYPE, GST_TREATMENT, INDUSTRY, LEAD_SOURCE, LEAD_STATUS, PLACE_OF_SUPPLY, RATING } from 'constants/app-constants';
import { SERVER_IP } from 'assets/Config';
import { postApi } from 'redux/sagas/postApiDataSaga';
import { resetApiStatus } from 'redux/reducers/globals/globalActions';
import { Checkbox } from 'antd';

const AddLead = ({ leadAddModal, width = '50%', editLead, setLeadAddModal, refreshList, handleClose }) => {
	...
	useEffect(() => {
		if (globalRedux.apiStatus.ADD_LEAD === 'SUCCESS' || globalRedux.apiStatus.EDIT_LEAD === 'SUCCESS') {
	...
	const loading = globalRedux.apiStatus.ADD_LEAD === API_STATUS.PENDING || globalRedux.apiStatus.EDIT_LEAD === API_STATUS.PENDING;

	return (
		<Drawer placement="right" title={`${editLead ? 'Edit' : 'New'} Lead`} width={width} open={leadAddModal} closable onClose={() => setLeadAddModal(false)} destroyOnClose size="large">
			<Form
				form={form}
				name="add-lead"
				layout="vertical"
				...
				<div style={{ fontWeight: 'bold', marginBottom: 16 }}>Lead Information</div>
				...
				<Row gutter={16}>
					...
					<Col span={12}>
						<Form.Item label="Company Name" name="companyname" rules={[{ required: true, message: 'Required!' }]}>
				...
				<Row gutter={16}>
					...
					<Col span={12}>
						<Form.Item label="Email Opt Out" name="rating">
							<Checkbox />
						</Form.Item>
					</Col>
					<Col span={12}>
				...
				<div style={{ fontWeight: 'bold', marginBottom: 16 }}>Address Information</div>
				...
				<div style={{ fontWeight: 'bold', marginBottom: 16, marginTop: 16 }}>Description Information</div>
				<Form.Item label="Description" name="description">
					<TextArea placeholder="Description" />
				</Form.Item>

				<Form.Item>
					<Row gutter={16} justify="space-between" style={{marginTop: 16, marginBottom:16}}>
						<Col span={12}>
