...
// const projectStatusTypes = ['Open', 'Assigned', 'Accepted', 'In Progress', 'Completed', 'Cancelled', 'Pending'];
...
const AssignToModal = ({ users, rowSelection, refreshList }) => {
	...
	const handleSubmit = (values) => {
		const request = rowSelection?.selectedRowKeys?.map((projectId, index) => ({ projectId })) ?? [];
		let url = `${SERVER_IP}project/assign?orgId=${organizationId}&assignedTo=${values?.assignTo}`;
		dispatch(postApi({ selectedProjects: request }, 'ADD_ASSIGNEE', url));
	};

	const handleStatusSubmit = (values) => {
		const request = rowSelection?.selectedRowKeys?.map((projectId, index) => ({ projectId })) ?? [];
		let url = `${SERVER_IP}project/status?orgId=${organizationId}`;
		dispatch(postApi({ selectedProjects: request, projectStatus: values?.status }, 'ADD_TICKET_STATUS', url));
	};
	...
	return (
		<>
			<Button disabled={rowSelection?.selectedRowKeys?.length === 0} onClick={() => setVisible(true)} style={{ marginLeft: 10 }} type="primary">
				Assign
			</Button>
			<Button disabled={rowSelection?.selectedRowKeys?.length === 0} onClick={() => setStatusVisible(true)} style={{ marginLeft: 10 }} type="primary">
				Update Status
			</Button>
			<Modal
